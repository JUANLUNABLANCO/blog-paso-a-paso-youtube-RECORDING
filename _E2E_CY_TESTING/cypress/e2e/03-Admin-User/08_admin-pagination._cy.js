  ...
  it('Creamos veinticinco usuarios en la base de datos, borrando los que ya había de anteriores tests, tendremos 25 usuarios aleatorios.', () => {
    ...
    USERS.forEach((user) => {
      cy.request({
        ...
        expect(resp.body.user).to.have.property('userName')
        expect(resp.body.user).to.have.property('email')
        expect(resp.body.user).to.have.property('id')
        expect(resp.body).to.have.property(Cypress.env('JWT_NAME'))
        // console.log('## DATA: ', resp.body)
      })
    })
    // creamos el admin
    cy.createAdminTokenSave().then((resp) => {
      // obtener el token de la localStorage
      Cypress.env('token', localStorage.getItem(`${Cypress.env('JWT_NAME')}`))
      const token = Cypress.env('token') // disponible en este bloque
      // headers Bearer token
      const authorization = `Bearer ${token}`

      // hagamos la petición con la cabecera del token en los headers
      cy.request({
        method: 'GET',
        url: `${Cypress.env('API_URL')}/api/users`, // por defecto la paginación envía 10, sino se especifica
        form: false,
        ...
        expect(resp.body.items.length).to.equal(10)
      })
      // Paginación con 50 de limit, solo podrá devolver los que hay en este caso 25 + admin = 26
      // hagamos la petición con la cabecera del token en los headers
      cy.request({
        method: 'GET',
        url: `${Cypress.env('API_URL')}/api/users?page=1&limit=50`, // por defecto la paginación envía 10, sino se especifica
        form: false,
        headers: { authorization },
      }).then((resp) => {
        expect(resp.body).to.have.property('items')
        ...
        expect(resp.body.items.length).to.equal(26)
        expect(resp.body.items[25].email).to.equal(Cypress.env('ADMIN_EMAIL'))
      })
    })
  })
...
//     cy.get('[data-test-id="usersTable"]')
//       .get('mat-row')
//       .eq(0)
//       .get('mat-cell')
//       .eq(1)
//       .contains('test') // accedemos a la tabla row[0] (registro 1) cell[1] columna name
//     cy.get('[data-test-id="usersTable"]')
//       .get('mat-row')
//       .eq(1)
//       .get('mat-cell')
//       .eq(1)
//       .contains('test') // accedemos a la tabla row[1] (registro 2) cell[1] columna name
//   })
// })
